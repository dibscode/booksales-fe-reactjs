...
export default function ShowBook() {
  ...
  const [quantity, setQuantity] = useState(1); // nilai default agar quantity bisa dibeli minimal 1 buku
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        alert(result.message + (result.data?.order_number ? `\nOrder: ${result.data.order_number}` : ""));
      } else {
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 block w-24 px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white focus:outline-none focus:ring-indigo-500 sm:text-sm"
                    />
                  </div>
